...
class EditTaskPage extends StatefulWidget {
  ...
  EditTaskPage({required this.tagsList, required this.tagsNameList, required this.Tasks, required this.task, required this.completedTasks});

  @override
  _TaskPageState createState() => _TaskPageState();
}

class _TaskPageState extends State<EditTaskPage> {
  ...
  void initState() {
    ...
    _taskNote = widget.task.text;
    _taskCategory = widget.task.tag.name;
    _taskDate = DateFormat('yyyy-MM-ddTHH:mm:ss').parse(widget.task.created);
    _priority = widget.task.priority;
    _hasDeadline = widget.task.finish != null;
    if (_hasDeadline) {
      _deadlineDate = DateFormat('yyyy-MM-ddTHH:mm:ss').parse(widget.task.finish!);
      _deadlineTime = TimeOfDay.fromDateTime(_deadlineDate);
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: _isLoading ? null : () async {
                  if (_formKey.currentState!.validate()) {
                    ...
                    Task newTask = Task(
                      ...
                      created: DateFormat('yyyy-MM-ddTHH:mm:ss').format(_taskDate),// Используем дату из _taskDate
                    );
                    if (_hasDeadline) {
                    ...
                    try{
                      ...
                      await _DataUpl.upDateTask(newTask);
                ...
                child: Text('Обновить', style: TextStyle(fontSize: 20,color: Colors.white,),),
